...
Coronary heart disease remains one of the leading causes of morbidity and mortality worldwide, with significant impacts on individuals, families, and healthcare systems. Our team chose this topic because understanding the factors associated with CHD is crucial for effective prevention, early detection, and management. By identifying modifiable risk factors and highlighting the importance of lifestyle choices, we aim to contribute to public health awareness and support evidence-based interventions that can reduce the burden of heart disease in our communities.

# Prior Research and Analysis

Extensive prior research has investigated the risk factors and determinants of coronary heart disease (CHD). Large-scale epidemiological studies, such as the [Framingham Heart Study](https://www.framinghamheartstudy.org/) and [INTERHEART](https://www.phri.ca/research/interheart/), have consistently identified age, sex, diabetes, smoking, obesity, physical inactivity, and poor self-reported health as major contributors to CHD risk. Meta-analyses and systematic reviews have further confirmed the strong associations between these modifiable and non-modifiable factors and the incidence of heart disease. The Behavioral Risk Factor Surveillance System (BRFSS) dataset has been widely used in public health research to monitor trends in cardiovascular risk factors and evaluate the effectiveness of interventions. Previous analyses using BRFSS and similar datasets have demonstrated that targeted lifestyle modifications - such as smoking cessation, increased physical activity, and weight management - can significantly reduce the burden of coronary heart disease at both individual and population levels. This study builds on this foundation by applying rigorous statistical methods to recent BRFSS data, aiming to clarify the relative importance of these factors in the current population.

# Dataset Description

This analysis utilizes data from the 2024 Behavioral Risk Factor Surveillance System (BRFSS), a nationwide health-related telephone survey conducted by the Centers for Disease Control and Prevention (CDC). The BRFSS collects information on health behaviors, chronic conditions, and preventive practices from a representative sample of adults across the United States. The 2024 dataset contains responses to over 300 variables, enabling comprehensive assessment of lifestyle, demographic, and clinical factors. For more details, the dataset is publicly available at [CDC BRFSS Annual Data 2024](https://www.cdc.gov/brfss/annual_data/annual_2024.html). This rich source of population-based data provides a robust foundation for investigating the determinants of coronary heart disease and other major health outcomes.
...
## New Smart Question 
# How do Age, Sex, Smoking_Status, Diabetes_Status, Exercise_Past_30_Days, and General_Health_Status affect the likelihood of having Coronary_Heart_Disease?

# Description
```{r}
summary(Heart_clean[, c("Coronary_Heart_Disease",
                    "BMI_Value",
                    ...
                    "Smoking_Status",
                    "BMI_Category_Alt")])
str(Heart_clean[, c("Coronary_Heart_Disease",
                    "BMI_Value",
                    "General_Health_Status",
                    "Exercise_Past_30_Days",
                    "Diabetes_Status",
...
**Rationale:**  
Logistic regression is appropriate for modeling a binary outcome (CHD: Yes/No) as a function of multiple predictors. We use class weights to address the imbalanced distribution of CHD cases.

**Hypothesis:**  
- Null hypothesis (H0): None of the predictors are associated with CHD.
- Alternative hypothesis (H1): At least one predictor is significantly associated with CHD.

```{r}
library(pscl)    
logit_1 <- glm(
  Coronary_Heart_Disease ~ Age + Sex + Smoking_Status + Diabetes_Status +
    Exercise_Past_30_Days + General_Health_Status,
  data = Heart_clean,
  family = binomial,
  weights = ifelse(Heart_clean$Coronary_Heart_Disease == "Yes", 20, 1)
)
...
*The logistic regression model shows that Age, Sex, Smoking_Status, Diabetes_Status, Exercise_Past_30_Days, and General_Health_Status are significant predictors of coronary heart disease. The model uses class weights (20:1) to address the imbalanced nature of the outcome variable.*
  
  ---
  
  # Model Evaluation - ROC-based Optimal Cutoff (RECOMMENDED)
  
  **Method:** We use ROC curve analysis to determine the optimal probability cutoff that maximizes sensitivity and specificity.

```{r}
library(pROC)

# Generate predicted probabilities
Heart_clean$pred_prob <- predict(logit_1, type = "response")

# ROC analysis
roc_obj <- roc(Heart_clean$Coronary_Heart_Disease, Heart_clean$pred_prob)
...
cat(sprintf("\nOptimal cutoff selected by ROC: %.4f\n", cutoff))

# Apply optimal cutoff
Heart_clean$pred_class <- ifelse(Heart_clean$pred_prob >= cutoff, "Yes", "No")
Heart_clean$pred_class <- factor(Heart_clean$pred_class, levels = c("No","Yes"))

# Confusion matrix
Logistic_conf_table <- table(Actual = Heart_clean$Coronary_Heart_Disease,
                             Predicted = Heart_clean$pred_class)

xkabledply(Logistic_conf_table, title = "Confusion Matrix from CHD Logistic Regression Model (ROC-based Cutoff)")

# Calculate metrics
Logistic_TN <- Logistic_conf_table[1,1]
Logistic_FP <- Logistic_conf_table[1,2]
Logistic_FN <- Logistic_conf_table[2,1]
Logistic_TP <- Logistic_conf_table[2,2]

accuracy  <- (Logistic_TP + Logistic_TN) / (Logistic_TP + Logistic_TN + Logistic_FP + Logistic_FN)
precision <- Logistic_TP / (Logistic_TP + Logistic_FP)
recall    <- Logistic_TP / (Logistic_TP + Logistic_FN)
...
kable(metrics_df, caption = "Logistic Regression Performance Metrics", align = 'c') %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```

*Using the ROC-based optimal cutoff (threshold), the logistic regression model achieves balanced performance across accuracy, precision, recall, and F1-score metrics.*
  ...
  ```{r}
cm_df <- as.data.frame(Logistic_conf_table)
cm_df$Actual <- factor(cm_df$Actual, levels = c("Yes", "No"))
...
ggplot(cm_df, aes(Predicted, Actual, fill = Freq)) +
  ...
  labs(
    ...
    subtitle = paste("Cutoff:", round(cutoff, 4)),
    x = "Predicted Class",
    y = "Actual Class") +
  theme_minimal()
```
...
---
  ...
  **Rationale:**  
  Decision trees provide an interpretable alternative to logistic regression. We use prior probabilities of 0.5/0.5 to address class imbalance.
...
tree_chd <- rpart(
  ...
  control = rpart.control(cp = 0.0001, minbucket = 1000)
)

# Predictions
pred_tree <- predict(tree_chd, type = "class")

# Confusion matrix
tree_conf_table <- table(
  Actual = Heart_clean$Coronary_Heart_Disease,
  Predicted = pred_tree
)

xkabledply(tree_conf_table, title = "Confusion Matrix from CHD Classification Tree")

# Calculate metrics
tree_TN <- tree_conf_table[1,1]
tree_FP <- tree_conf_table[1,2]
tree_FN <- tree_conf_table[2,1]
tree_TP <- tree_conf_table[2,2]
...
tree_metrics_df <- data.frame(
  Metric = c("Accuracy", "Precision", "Recall", "F1-Score"),
  Value = round(c(accuracy, precision, recall, f1_score), 4)
)

kable(tree_metrics_df, caption = "Classification Tree Performance Metrics", align = 'c') %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```
...
rpart.plot(
  ...
  fallen.leaves = TRUE,
  cex = 0.7,
  main = "Classification Tree for Coronary Heart Disease")

# Confusion matrix heatmap
cm_tree_df <- as.data.frame(tree_conf_table)

ggplot(cm_tree_df, aes(Predicted, Actual, fill = Freq)) +
  geom_tile() +
  geom_text(aes(label = Freq), color = "white", size = 6) +
  scale_fill_gradient(low = "skyblue", high = "darkblue") +
  labs(title = "Confusion Matrix of Classification Tree",
       x = "Predicted Class",
       y = "Actual Class") +
  theme_minimal()
```
...
**Logistic Regression vs. Classification Tree:**
  ...
  - **Logistic regression** captures smooth, continuous relationships between predictors and the outcome, making it well-suited for modeling the probability of CHD based on multiple risk factors. It performs well on imbalanced datasets when combined with class weights or optimal cutoff selection.

- **Classification trees** use hard decision boundaries and may underfit when the dataset is highly imbalanced, even with prior adjustments. However, they offer greater interpretability through visual representation of decision rules.

**Conclusion:**  
  In this analysis, logistic regression demonstrates superior performance in terms of accuracy, precision, recall, and F1-score compared to the classification tree. The use of ROC-based optimal cutoff further enhances the model's ability to balance sensitivity and specificity.