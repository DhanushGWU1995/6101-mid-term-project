...
author: |
  ...
  Dhanush Mathivanan, Heng Jui Chu, Nikhilesh Dhavale, Aditya Karande
date: "`r Sys.Date()`"
output:
  html_document:
    toc: true
...
train_index <- createDataPartition(Heart_clean$Coronary_Heart_Disease, p = 0.7, list = FALSE)
...
*The random forest model was trained using 500 trees, with 2 variables tried at each split (mtry), and a minimum node size of 5 to allow for finer splits given the small dataset size. The model uses permutation importance for interpretability and samples 70% of the training data for each tree to enhance robustness.*

*The out-of-bag Brier score is 0.167. This is the mean squared error of the predicted probabilities vs. actual outcomes (lower is better; 0 is perfect, 0.25 is random guessing for balanced classes).*
...
*The optimal Complexity Parameter of 0.00105 is a cost-complexity pruning metric selected via 10-fold cross-validation that balances tree complexity with accuracy, preventing overfitting to training noise while improving generalization to new patients and enhancing interpretability with clinical decision rules doctors can easily follow, this value represents the sweet spot between underfitting (CP too high 0.1) and overfitting (CP too low 0.0001), ensuring that when the tree is pruned, only the most meaningful and predictive splits are retained, transforming a complex full tree into a robust, interpretable model suitable for real-world CHD risk assessment.*
...
```{r, fig.width=10, fig.height=6}
# Create ROC objects for both models
roc_rf <- pROC::roc(test_rf$Coronary_Heart_Disease, pred_prob_orig, 
                    direction = "<", levels = c("No", "Yes"))
roc_tree <- pROC::roc(test_rf$Coronary_Heart_Disease, tree_pred_prob,
                      direction = "<", levels = c("No", "Yes"))

# Plot Random Forest ROC curve
plot(roc_rf, col = "#1c61b6", lwd = 3, 
     main = "ROC Curve Comparison: Random Forest vs Decision Tree",
     xlab = "Specificity", 
     ylab = "Sensitivity",
     legacy.axes = FALSE,  # Medical format: Specificity 1 to 0, Sensitivity 0 to 1
     xlim = c(1, 0),  # X-axis goes from 1 (left) to 0 (right) very few false positives to many false positives
     ylim = c(0, 1), # Y-axis goes from 0 (bottom) to 1 (top) very few true positives to many true positives
     identity = FALSE,
     print.auc = FALSE) 
...
segments(x0 = 1, y0 = 0, x1 = 0, y1 = 1, lty = 2, col = "gray", lwd = 2)
