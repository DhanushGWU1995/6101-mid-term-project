...
# Step 5: Handle missing values in BMI Category (categorical - no imputation)
cat("\n=== HANDLE MISSING VALUES: BMI Category ===\n")

if("BMI_Category_Alt" %in% names(Heart_clean)) {
  missing_bmi_cat <- sum(is.na(Heart_clean$BMI_Category_Alt))
  missing_bmi_pct <- (missing_bmi_cat / nrow(Heart_clean)) * 100

  cat(sprintf("BMI_Category_Alt: %d missing (%.1f%%)\n", missing_bmi_cat, missing_bmi_pct))

  if(missing_bmi_cat > 0) {
    cat("Removing rows with missing BMI_Category_Alt (complete case analysis)\n")
    rows_before <- nrow(Heart_clean)
    Heart_clean <- Heart_clean[!is.na(Heart_clean$BMI_Category_Alt), ]
...
Is there a significant difference in BMI between people with and without coronary heart disease? (T-test)
...
ggplot(Heart_clean, aes(x = Coronary_Heart_Disease, y = BMI_Value, fill = Coronary_Heart_Disease)) +
  geom_boxplot() +
  labs(title = "BMI vs. Coronary Heart Disease",
...
Is smoking status associated with coronary heart disease? (Chi-square test)
...
ggplot(Heart_clean, aes(x = Smoking_Status, fill = Coronary_Heart_Disease)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  labs(title = "CHD Proportion vs. Smoking Status",
       x = "Smoking Status",
       y = "Percentage",
       fill = "CHD") +
  theme_minimal()
...
Does BMI vary significantly across different general health status levels? (ANOVA)
...
ggplot(Heart_clean, aes(x = General_Health_Status, y = BMI_Value, fill = General_Health_Status)) +
  geom_boxplot() +
  labs(title = "BMI vs. General Health Status",
...
Does exercise participation vary significantly across different BMI categories, suggesting a role in weight management? (Chi-square test)
...
ggplot(Heart_clean, aes(x = BMI_Category_Alt, fill = Exercise_Past_30_Days)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  labs(title = "Exercise Participation vs. BMI Category",
       x = "BMI Category",
       y = "Percentage",
       fill = "Exercised in Past 30 Days") +
  theme_minimal()
```
...
Is diabetes status associated with coronary heart disease? (Chi-square test)
...
ggplot(Heart_clean, aes(x = Diabetes_Status, fill = Coronary_Heart_Disease)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  labs(title = "Coronary Heart Disease vs. Diabetes Status",
       x = "Diabetes Status",
       y = "Percentage",
       fill = "CHD Status") +
  theme_minimal()
