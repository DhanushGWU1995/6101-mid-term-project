...
## New Smart Question 
# How do Age_Group_5yr, Sex, Smoking_Status, Diabetes_Status, and Exercise_Past_30_Days affect the likelihood of having Coronary_Heart_Disease?
# Description
```{r}
...
Heart_clean$pred_class <- ifelse(Heart_clean$pred_prob >= 0.005, "Yes", "No")
...
xkabledply(Logistic_conf_table, title = "Confusion Matrix from CHD Logistic Regression Model")
...
data.frame(accuracy, precision, recall, f1_score)
```

# accuracy precision recall f1_score (Cut off way!)
```{r}
library(pROC)
...
Logistic regression performs better because it captures smooth, continuous relationships in the data.

The classification tree performs worse because it uses hard splits and tends to underfit when the dataset is highly imbalanced.                                                                                                                                                                                                                                                                                                                                                                          
```{r}
library(rpart.plot)
...
rpart.plot(
  ...
  fallen.leaves = TRUE,
  cex = 0.7,
  main = "Category Tree for CHD")


library(ggplot2)

cm_df <- as.data.frame(tree_conf_table)

ggplot(cm_df, aes(Predicted, Actual, fill = Freq)) +
  geom_tile() +
  geom_text(aes(label = Freq), color = "white") +
  scale_fill_gradient(low = "skyblue", high = "darkblue") +
  labs(title = "Confusion Matrix of Category Tree") +
  theme_minimal()
```