...
library(pscl)    
logit_1 <- glm(
  Coronary_Heart_Disease ~ Age + Sex + Smoking_Status + 
...
Heart_clean$pred_class <- ifelse(Heart_clean$pred_prob >= 0.5, "Yes", "No")
Heart_clean$pred_class <- factor(Heart_clean$pred_class, levels = c("No","Yes"))

conf_table <- table(Actual = Heart_clean$Coronary_Heart_Disease,
                    Predicted = Heart_clean$pred_class)

xkabledply(conf_table, title = "Confusion Matrix from CHD Logistic Regression Model")
